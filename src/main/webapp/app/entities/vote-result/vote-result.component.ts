...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/vote-result'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: VoteResult) {
        return item.id;
    }
    registerChangeInVoteResults() {
        this.eventSubscriber = this.eventManager.subscribe('voteResultListModification', (response) => this.loadAll());
    }
